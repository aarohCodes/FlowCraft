...
import { videoMeetingApi, VideoMeeting, VideoParticipant } from '../../services/videoMeetingApi';
...
export const VideoMeetingIntegration: React.FC = () => {
  ...
  useEffect(() => {
    if (state.isZoomConnected) {
      loadMeetings();
      setConnectedPlatforms(videoMeetingApi.getConnectedPlatforms());
    }
  ...
  const handleConnect = async (platform: string) => {
    try {
      ...
      const authUrl = await videoMeetingApi.authorize(platform as any);
      window.open(authUrl, '_blank', 'width=500,height=600');
      
      // Listen for the callback
      const handleMessage = (event: MessageEvent) => {
        if (event.data.type === 'VIDEO_MEETING_AUTH_SUCCESS' && event.data.platform === platform) {
          if (!state.isZoomConnected) {
      ...
      toast.error(`Failed to connect to ${platformNames[platform as keyof typeof platformNames]}`);
      console.error('Video meeting connection error:', error);
    }
  };

  const handleDisconnect = (platform?: string) => {
    if (platform) {
      videoMeetingApi.disconnectPlatform(platform);
      toast.success(`Disconnected from ${platformNames[platform as keyof typeof platformNames]}`);
    } else {
      videoMeetingApi.disconnectAll();
      dispatch({ type: 'TOGGLE_ZOOM_CONNECTION' });
  ...
  const loadMeetings = async () => {
    setLoading(true);
    try {
      const allMeetings = await videoMeetingApi.getAllMeetings();
  ...
  const startMonitoring = async (meeting: VideoMeeting) => {
    setActiveMeeting(meeting);
    setIsMonitoring(true);
    
    try {
      await videoMeetingApi.startRealTimeMonitoring(meeting.id, (data) => {
      ...
      toast.success(`Monitoring ${meeting.topic} on ${platformNames[meeting.platform as keyof typeof platformNames]}`);
    } catch (error) {
  ...
  const stopMonitoring = () => {
    videoMeetingApi.stopRealTimeMonitoring();
    setIsMonitoring(false);
    setActiveMeeting(null);
    setParticipants([]);
  ...
  const generateInsights = async (meeting: VideoMeeting) => {
    setLoading(true);
    try {
      const meetingInsights = await videoMeetingApi.generateMeetingInsights(meeting.id);
  ...
  const generateFlashcards = async (meeting: VideoMeeting) => {
    setLoading(true);
    try {
      const flashcards = await videoMeetingApi.generateFlashcardsFromMeeting(meeting.id);
      ...
      toast.success(`Generated ${flashcards.length} flashcards from ${platformNames[meeting.platform as keyof typeof platformNames]} meeting!`);
    } catch (error) {
  ...
  const createMeeting = async (platform: string) => {
    try {
      const meeting = await videoMeetingApi.createMeeting(platform, {
      ...
      toast.success(`Meeting created on ${platformNames[platform as keyof typeof platformNames]}!`);
    } catch (error) {
      ...
      >
        <div className="flex items-center space-x-3 mb-6">
          ...
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">
            {platformNames[selectedPlatform as keyof typeof platformNames]} Setup Required
          </h3>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-800 dark:text-blue-300 text-sm">
                  Due to the dynamic nature of this environment, you'll need to update your app settings for {platformNames[selectedPlatform as keyof typeof platformNames]}.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900 dark:text-white">
              To connect your real {platformNames[selectedPlatform as keyof typeof platformNames]} account:
            </h4>
            <ol className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
              <li className="flex items-start space-x-3">
                ...
                <div>
                  <p>Go to your {platformNames[selectedPlatform as keyof typeof platformNames]} developer console</p>
                </div>
              </li>
              <li className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <code className="block bg-gray-100 dark:bg-gray-700 px-3 py-2 rounded mt-1 text-xs break-all">
                    {window.location.origin}/video-meeting/callback/{selectedPlatform}
                  </code>
                </div>
              </li>
              <li className="flex items-start space-x-3">
    ...
    return (
      ...
      >
        ...
        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
          Connect Video Meeting Platforms
        </h3>
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          Connect your video meeting accounts to enable real-time monitoring, 
          transcript analysis, and AI-powered insights across all platforms.
        </p>
      ...
      <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-xl p-6 border border-gray-200 dark:border-gray-700 shadow-lg">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-gray-900 dark:text-white font-medium">
              Connected Platforms {connectedPlatforms.some(p => p.includes('mock_access_token')) && '(Demo Mode)'}
            </span>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {connectedPlatforms.map((platform) => (
            <div key={platform} className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
              ...
              <div className="flex items-center space-x-2">
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => createMeeting(platform)}
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <span className="text-lg">{platformIcons[activeMeeting.platform as keyof typeof platformIcons]}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                {platformNames[activeMeeting.platform as keyof typeof platformNames]}
              </span>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <span className="text-lg">{platformIcons[meeting.platform as keyof typeof platformIcons]}</span>
                    <h4 className="font-medium text-gray-900 dark:text-white">
                      {meeting.topic}
                    </h4>
                    <span className="text-xs text-gray-500 dark:text-gray-400 bg-gray-200 dark:bg-gray-600 px-2 py-1 rounded-full">
                      {platformNames[meeting.platform as keyof typeof platformNames]}
                    </span>
                  </div>
      ...
      <AnimatePresence>
        {insights && (
          ...
          >
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                <span className="text-lg">{platformIcons[insights.platform as keyof typeof platformIcons]}</span>
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                  Meeting Insights - {platformNames[insights.platform as keyof typeof platformNames]}
                </h3>
              </div>
              <button
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-900 dark:text-white">{platformNames[insights.platform as keyof typeof platformNames]}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600 dark:text-gray-400">Duration:</span>
                    <span className="text-gray-900 dark:text-white">{insights.duration} min</span>
                  </div>
                  <div className="flex justify-between">
